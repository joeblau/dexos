...
/// A single signed price observation from one oracle venue/signer.
...
pub struct PriceObservation {
    ...
    pub price: Price,
    /// Confidence / liquidity weight behind the price (micro-units).
    pub confidence: Amount,
    /// Bitmap of contributing raw venues; popcount is the source-diversity count.
    pub source_mask: u64,
...
impl PriceObservation {
    ...
    /// Number of distinct raw venues behind this observation.
    #[inline]
    pub fn source_count(&self) -> u32 {
        self.source_mask.count_ones()
    }
    ...
    pub fn signing_digest(&self) -> Hash {
        let mut buf = Vec::with_capacity(4 + 8 + 16 + 8 + 8 + 8);
        ...
        buf.extend_from_slice(&self.source_mask.to_le_bytes());
