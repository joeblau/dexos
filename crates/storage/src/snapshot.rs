...
impl Snapshot {
    ...
    /// Decode a snapshot from a byte frame, verifying its integrity.
    ///
    /// This is total: any input yields `Ok` or a typed [`SnapshotError`], never
    /// a panic. A truncated frame, a flipped bit, or a state/digest mismatch all
